...
export class Engine {
  ...
  clearCanvas() {
    ...
    if (this.selectedTool === "pointer" && this.existingShapes.length <= 0) {
      ...
      this.ctx.fillStyle = "#7c7c7c";
      this.ctx.fillText("Clear Canvas and change backgrounds", 85, 152);
      ...
      this.ctx.fillText("Drawception", window.innerWidth / 2 - 160, window.innerHeight / 2);
  ...
  initMouseHanlders() {
    ...
    this.mouseMoveHandler = (e) => {
      ...
      if (this.clicked) {
        ...
        if (this.selectedTool === "rect") {
        ...
        else if (this.selectedTool === "eraser" && this.clicked) {
          const x = this.transformX(e.clientX);
          const y = this.transformY(e.clientY);
          const eraseRadius = 8; // Eraser size
          ...
          for (let shape of this.existingShapes) {
            if (shape.type === "pencil") {
              let segments = [];
              let currentSegment = [];
              for (let point of shape.cords) {
                let dist = Math.hypot(point.x - x, point.y - y);
                if (dist > eraseRadius) {
                  currentSegment.push(point);
                } else {
                  // When entering eraser zone, save current segment
              ...
              // Push last remaining segment if any
              if (currentSegment.length > 0) {
                segments.push(currentSegment);
              }
              // Each segment becomes its own pencil stroke
              for (let seg of segments) {
                if (seg.length > 1) {
                  newShapes.push({
                    ...shape,
                    id: Date.now() + Math.random(), // New ID so canvas treats it as new stroke
                    cords: seg
                  });
                }
              }
              ...
              // Other shapes: erase entirely if touched
              if (!this.isPointInShape(x, y, shape)) {
